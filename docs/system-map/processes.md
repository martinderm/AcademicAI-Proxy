...
| `test_cost_calculation_unit.py` | Unit-Tests für 24h-Modellkatalog (`ModelCatalog`) und hochpräzise `Decimal`-Kostenberechnung (`calculate_request_cost`): 1k-Token-Normalisierung, 24h-TTL, atomare JSON-Dateipersistenz, per-request-Kosten, Tiered-Model-Erkennung, Token-Grenzwerte (`context_window`, `output_token_limit`), OpenAI-Format-Generierung und Header-Formatierung. |
